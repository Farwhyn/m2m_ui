...
//					System.err.println("slices flushed");
				...
				for(int sliceNumber = lastRenderedSlice + 1; sliceNumber <= lastSliceIndex; sliceNumber++) {
					...
					for(Dataset dataset : datasets) {
						...
						double x = (double) firstIndex * (double) graphWidth / (double) duration - (sliceNumber * sliceWidth);
						double y = (double) graphHeight - (dataset.get(firstIndex) - minY) / range * (double) graphHeight;
						path.moveTo(x, y);
