...
switch lower(mode)
	case 'initialize'
	    initialize(hLine);  %INITIALIZE(HAXES)
	    
	case 'update'
		update(hLine);  %UPDATE(HLINE,xnew,ynew)
	otherwise
...
hold all
for m = 1:numlines
    plot(hLine,dummydata,dummydata);
end
...
hChildren=get(hLine,'Children');

xData = get(hChildren(1),'XData');     % old x data
yData = get(hChildren(1),'YData');     % old y data
...
set(hChildren(1),'XData',xData)                % update plot
set(hChildren(1),'YData',yData)                % update plot
